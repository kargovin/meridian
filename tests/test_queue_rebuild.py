...
def test_a_healthy_queue_is_missing_nothing(
    app_session: Session, populated: dict[str, int]
) -> None:
    assert missing_article_work(app_session) == set()


def test_queue_survives_being_truncated(app_session: Session, populated: dict[str, int]) -> None:
    """AC4. Every article that owed work still owes it after the queue is thrown away."""
    before = open_article_work(app_session)
    assert before == expected_article_work(app_session)

    app_session.execute(sa.text("TRUNCATE pipeline_work RESTART IDENTITY"))
    app_session.commit()

    assert open_article_work(app_session) == set()
    assert expected_article_work(app_session) == before
    assert missing_article_work(app_session) == before


def test_finished_and_terminal_articles_owe_nothing(
    app_session: Session, populated: dict[str, int]
) -> None:
    owed = {article_id for article_id, _ in expected_article_work(app_session)}
...
def test_a_dropped_enqueue_is_reported(app_session: Session, populated: dict[str, int]) -> None:
