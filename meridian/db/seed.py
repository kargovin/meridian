...
⚠️ Seeding inserts and never updates. A source already in the registry is left exactly as it
is, including its ``enabled`` and ``rights_level``. Those are the two fields an operator
changes under pressure — a Legal call, a ToS complaint — and a seed that reconciled the
registry towards a file would silently undo that on the next deploy. The registry is
...
def parse(raw: object) -> list[SeedEntry]:
    ...
    if not isinstance(raw, list):
        raise ValueError("a roster is a JSON array of source objects")
    entries = []
...
def seed(session: Session, entries: Iterable[SeedEntry]) -> tuple[list[str], list[str]]:
    """Insert every entry the registry does not already hold.

    Returns the names inserted and the names skipped, so a caller can report both rather than
    a single count that hides which half happened.
    """
    ...
    for entry in entries:
        ...
        sources.create(
            ...
            acquisition_tier=entry.acquisition_tier,
